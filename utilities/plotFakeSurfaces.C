...
void loopOverExperimentsNormalOrInverted(int normalOrInverted, int numExps,TFile *fpInRun[],TH2D *histFour[], TH2D *histTotal, TH2D *histNQPQ );
...
void plotFakeSurfaces(int fakeDmBin, int fakeT23bin,int fakeT13Bin, int fakeDeltaBin, int numExps)
{
  ...
  for(int runInd=0;runInd<11;runInd++) {
    ...
    sprintf(fileName,"fakeSurfaces/fakeSurface_%s_exps_%d_%d_%d_%d_%d.root",runNameArray[runInd],NUM_EXPS,fakeDmBin,fakeT23bin,fakeT13Bin,fakeDeltaBin);
  ...
  ///Add here RJN
  loopOverExperimentsNormalOrInverted(0, numExps,fpInRun,histFourNormal,histTotalNormal, histNQPQNormal );
  loopOverExperimentsNormalOrInverted(1, numExps,fpInRun,histFourInverted,histTotalInverted, histNQPQInverted );
...
//   OutText.close();
  ...
  canCont->Update();
  canCont->Modified();
  sprintf(canName,"plots/canContAll_%d_%d_%d_%d_%d.pdf",numExps,fakeDmBin,fakeT23bin,fakeT13Bin,fakeDeltaBin);//,plotT13Bin,plotDeltaBin);
  canCont->Print(canName);


  canNormalSurface->Update();
  canNormalSurface->Modified();
  sprintf(canName,"plots/canNormalSurface_%d_%d_%d_%d_%d.pdf",numExps,fakeDmBin,fakeT23bin,fakeT13Bin,fakeDeltaBin);//,plotT13Bin,plotDeltaBin);
  canNormalSurface->Print(canName);

  canNormalPlusNC->Update();
  canNormalPlusNC->Modified();
  sprintf(canName,"plots/canNormalPlusNC_%d_%d_%d_%d_%d.pdf",numExps,fakeDmBin,fakeT23bin,fakeT13Bin,fakeDeltaBin);//,plotT13Bin,plotDeltaBin);
  canNormalPlusNC->Print(canName);
...
void loopOverExperimentsNormalOrInverted(int normalOrInverted, int numExps,TFile *fpInRun[],TH2D *histFour[], TH2D *histTotal, TH2D *histNQPQ )
{
  ...
  for(int exp=0;exp<numExps;exp++) {
    ...
    if(normalOrInverted==0) {
      if(llArrayNormal[exp]>minLL || llArrayNormal[exp]<0) {
	t23ArrayNormal[exp]=minT23;
	dm2ArrayNormal[exp]=minDm2;
	llArrayNormal[exp]=minLL;
      }
    }
    else {
      if(llArrayInverted[exp]>minLL || llArrayInverted[exp]<0) {
	t23ArrayInverted[exp]=minT23;
	dm2ArrayInverted[exp]=minDm2;
	llArrayInverted[exp]=minLL;
      }
    }
