...
void doGridSearchFake(int run, int fakeDmBin, int fakeT23bin, int fakeT13Bin, int fakeDeltaBin,int potScale);
...
int main(int argc, char **argv) 
{
  ...
  doGridSearchFake(run,dmBin,t23Bin,t13Bin,deltaBin,potScale);
...
void doGridSearchFake(int run, int fakeDmBin, int fakeT23bin, int fakeT13Bin, int fakeDeltaBin, int potScale)
{
  ...
  for(int fileType=0;fileType<3;fileType++) { 
    ...
    std::cout << runNameArray[run-1] << " " << fileTypeTag[fileType] << "\t" << dataPOT << "\t" << mcPOT[fileType] << "\t" << scaleFactor[fileType] << "\n";
  }
  ...
  Float_t *tempArray[4][NUM_EXPS];
  for(int i=0;i<4;i++) {
     for(int exp=0;exp<NUM_EXPS;exp++) {
	tempArray[i][exp] = new Float_t[MAX_DMI_INDEX*MAX_T23_INDEX*MAX_T13_INDEX*MAX_DELTA_INDEX];
     }
  }
...
// 	      }
	      ...
	      tempArray[histType][exp][getTempIndex(dmi,t23i,t13i,deltai)]+=LL;
  ...
  for(int histType=0;histType<4;histType++) {
    for(int exp=0;exp<NUM_EXPS;exp++) {	
      if(!histData[histType][exp]) break;
      fpExp[exp]->cd();

      sprintf(outName,"histBestInverted_%s_%d",histNameArray[histType],exp);
      TH2D histBestInverted(outName,outName,MAX_T23_INDEX,MIN_T23-0.5*T23_STEP_SIZE,MIN_T23+(MAX_T23_INDEX-0.5)*T23_STEP_SIZE,
			    MAX_DMI_INDEX/2,1e3*(MIN_NEG_DM2-0.5*DM2_STEP_SIZE),1e3*(MIN_NEG_DM2+(MAX_DMI_INDEX/2)*DM2_STEP_SIZE));
      sprintf(outName,"histBestNormal_%s_%d",histNameArray[histType],exp);
      TH2D histBestNormal(outName,outName,MAX_T23_INDEX,MIN_T23-0.5*T23_STEP_SIZE,MIN_T23+(MAX_T23_INDEX-0.5)*T23_STEP_SIZE,
	...
	for(int t23i=0;t23i<MAX_T23_INDEX;t23i++) {
      ...
      for(int t13i=0;t13i<MAX_T13_INDEX;t13i++) {
	for(int deltai=0;deltai<MAX_DELTA_INDEX;deltai++) {
	  sprintf(outName,"histInverted_%s_%d_%d_%d",histNameArray[histType],exp,t13i,deltai);
	  TH2D histInverted(outName,outName,MAX_T23_INDEX,MIN_T23-0.5*T23_STEP_SIZE,MIN_T23+(MAX_T23_INDEX-0.5)*T23_STEP_SIZE,
			    MAX_DMI_INDEX/2,1e3*(MIN_NEG_DM2-0.5*DM2_STEP_SIZE),1e3*(MIN_NEG_DM2+(MAX_DMI_INDEX/2)*DM2_STEP_SIZE));
	  sprintf(outName,"histNormal_%s_%d_%d_%d",histNameArray[histType],exp,t13i,deltai);
	  ...
	  for(int dmi=0;dmi<MAX_DMI_INDEX;dmi++) {
	    for(int t23i=0;t23i<MAX_T23_INDEX;t23i++) {
	      Double_t LL=tempArray[histType][exp][getTempIndex(dmi,t23i,t13i,deltai)];
