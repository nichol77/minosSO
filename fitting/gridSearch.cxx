...
void doGridSearchGeneric(int run,char *inFile, char *outFile,char *histTag, int potScale, int fakeData)
{
  ...
  for(int histType=0;histType<4;histType++) {	  	
     if(fakeData) {	
	sprintf(histName,"%s_%s",histNameArray[histType],histTag); 
     }
     else {
	sprintf(histName,"%s_%s_%s",histNameArray[histType],histNameTagArray[run-1],histTag);     
     }
