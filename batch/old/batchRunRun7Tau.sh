...
for (( i=0; i<${tLen}; i+=8 ));
do
  echo "${fileArray[$i]} ${fileArray[$i+1]} ${fileArray[$i+2]} ${fileArray[$i+3]} ${fileArray[$i+4]} ${fileArray[$i+5]} ${fileArray[$i+6]} ${fileArray[$i+7]}"
    outfile=/unix/minos1/rjn/ccPlusNc/farPreds/run7/tau${outcount}.root
    echo $outfile;
