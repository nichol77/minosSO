...
do
  ...
  echo "qsub -q short -M rjn@hep.ucl.ac.uk ~/minos/ccPlusNc/batch/multiConcatanate.sh -v INPUTFILE1=${fileArray[$i]},INPUTFILE2=${fileArray[$i+1]},INPUTFILE3=${fileArray[$i+2]},INPUTFILE4=${fileArray[$i+3]},OUTPUTFILE=${outfile}"
sleep 1
done
