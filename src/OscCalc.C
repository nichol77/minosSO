...
void OscCalc::SetOscParam(OscPar::OscPar_t pos, double val, bool force)
{
  if(fabs(fOscPar[pos] - val) > 1e-9 || force){
    ...
    if(pos < 3){
      //       fSinTh[pos] = sin(val);
      //       fCosTh[pos] = cos(val);
      sincos(val,&fSinTh[pos],&fCosTh[pos]);
      //       fSin2Th[pos] = sin(2*val);
      //       fCos2Th[pos] = cos(2*val);
      sincos(2*val,&fSin2Th[pos],&fCos2Th[pos]);
    }
...
double OscCalc::MuToElec(double x)
{
  ...
  double cosC13Delta = 0;// cos(C13*Delta);
  double sinC13Delta = 0;//sin(C13*Delta);
  sincos(C13*Delta,&sinC13Delta,&cosC13Delta);
  
  double sin1pADelta = 0;//sin((A+1)*Delta);
  double cos1pADelta = 0;//cos((A+1)*Delta);
  sincos((A+1)*Delta,&sin1pADelta,&cos1pADelta);

  double sinADelta = sin(A*Delta);
  double sinAm1Delta = sin((A-1)*Delta);
  double cosdpDelta = cos(d_cp+Delta);
  double sinApam2Delta = 0;//sin((A+alpha-2)*Delta);
  double cosApam2Delta = 0;//cos((A+alpha-2)*Delta);
  sincos((A+alpha-2)*Delta,&sinApam2Delta,&cosApam2Delta);
  ...
  if(fabs(alpha) > 1e-10){
    ...
    sincos(alpha*C12*Delta,&sinaC12Delta,&cosaC12Delta);
...
double OscCalc::MuToTau(double x)
{
  ...
  if(fabs(alpha) > 1e-10){
    cosaC12Delta = 0;//cos(alpha*C12*Delta);
    sinaC12Delta = 0;//sin(alpha*C12*Delta);
    sincos(alpha*C12*Delta,&sinaC12Delta,&cosaC12Delta);
  } // otherwise not relevant
  ...
  double sinAm1Delta = 0;//sin((A-1)*Delta);
  double cosAm1Delta = 0;//cos((A-1)*Delta);
  sincos((A-1)*Delta,&sinAm1Delta,&cosAm1Delta);
...
double OscCalc::MuToMu(double x)
{
  ...
  double p1 = 1. - pMuToTau - pMuToElec;
  if(p1 < 1e-6){ 
